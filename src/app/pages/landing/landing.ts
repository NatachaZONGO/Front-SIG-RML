...
export class Landing implements OnInit {
  ...
  // ✅ Données des offres, publicités et conseils
  recentOffers: Offre[] = [];
  featuredJobs: Offre[] = [];
  adsRow: Publicite[] = [];
  tipsRow: Conseil[] = [];
  
  // ✅ Map pour stocker les offres par ID
    ...
    this.conseilApi.getConseils(1, 12).subscribe({
      next: (res) => {
        ...
        this.tipsRow = list
          ...
          .sort((a, b) => {
            const dateA = a.date_publication ? new Date(a.date_publication as any).getTime() : 0;
            const dateB = b.date_publication ? new Date(b.date_publication as any).getTime() : 0;
            return dateB - dateA;
          })
          .slice(0, 6);
      },
...
private decodeHtml(html: string): string {
  ...
  // ✅ Méthode pour copier le lien court
  ...
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(link).then(() => {
      this.messages.add({
        severity: 'success',
        summary: '✓ Lien copié',
        detail: 'Le lien de l\'offre a été copié dans le presse-papier',
        life: 3000
      });
    }).catch(() => {
...
// ✅ Méthode pour générer le lien court
